<template>
  <section
    ref="heroSection"
    class="relative flex items-center justify-center content-center flex-wrap bg-designer text-white"
    :class="{'h-screen': mobileFullscreen }"
  >
    <span
    ...
    <video v-if="mobileVideo" ref="mobileVideoRef" src="~/assets/1024x576.mp4"></video>
    <video v-if="tabletVideo" ref="tabletVideoRef" src="~/assets/1280x720.mp4"></video>
    <video v-if="desktopVideo" ref="desktopVideoRef" src="~/assets/1920x1080.mp4"></video>

    <article
      class="w-full flex justify-center flex-wrap absolute"
    >
      <h1
      ...
      >
        <h2 class="font-thin">I work as</h2>
        <h2
          ref="devElemTitle"
          class="font-dev font-light"
...
export default {
  ...
  mounted() {
    ...
    if (window.innerWidth < 1023) {
      ...
      this.tabletVideo = false
      this.desktopVideo = false
      const { mobileVideoRef } = this.$refs
      console.log(mobileVideoRef)
      mobileVideoRef.oncanplaythrough = (event) => {
        setTimeout( () => { 
          mobileVideoRef.play()
         }, 2300 )
      }
    ...
    if (window.innerWidth < 1279) {
      ...
      this.tabletVideo = true
      this.desktopVideo = false
      const { tabletVideoRef } = this.$refs
      console.log(tabletVideoRef)
      tabletVideoRef.oncanplaythrough = (event) => {
        setTimeout( () => { 
          tabletVideoRef.play()
         }, 2300 )
      }
    ...
    if (window.innerWidth > 1279) {
      ...
      const { desktopVideoRef } = this.$refs
      console.log(desktopVideoRef)
      desktopVideoRef.oncanplaythrough = (event) => {
        setTimeout( () => { 
